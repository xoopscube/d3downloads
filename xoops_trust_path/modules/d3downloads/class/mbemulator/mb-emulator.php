...
Class HypMBString
{
	...
	{
		$max = preg_match_all('/'.$this->mbemu_internals['regex'][2].'/', $str, $allchars);  // ʸ���������ʬ��
		$str_EUC = '';
		for ($i = 0; $i < $max; ++$i) {
			$num = ord($allchars[0][$i]);  // ��ʸ����1�Х����ܤ���ͤȤ��Ƽ��Ф�
			if ($num2 = ord($allchars[1][$i])) { // 2�Х����ܤ�������
	...
	{
		$max = preg_match_all('/'.$this->mbemu_internals['regex'][1].'/', $str, $allchars);  // ʸ���������ʬ��
		$str_SJIS = '';
		for ($i = 0; $i < $max; ++$i) {
			$num = ord($allchars[0][$i]);  // ��ʸ����1�Х����ܤ���ͤȤ��Ƽ��Ф�
			if ($num2 = ord($allchars[1][$i])) { // �����ξ��
			...
			} elseif ($num3 = ord($allchars[2][$i])) {//Ⱦ�ѥ���
				$str_SJIS .= chr($num3);
			} else { //�ѿ���
				$str_SJIS .= chr($num);
			}
	...
	{
		$max = preg_match_all('/'.$this->mbemu_internals['regex'][3].'/', $str, $allchunks, PREG_SET_ORDER);  // ʸ���老�Ȥ������ʬ��
		$st = '';
		for ($i = 0; $i < $max; ++$i) {
			if (ord($allchunks[$i][1])) { //�ѿ��˥ޥå�
				$st .= $allchunks[$i][1];
			} elseif (ord(@$allchunks[$i][2])) { //�����˥ޥå�
				$tmp = substr($allchunks[$i][0], 3, strlen($allchunks[$i][0]));
				$st .= preg_replace_callback("/.(.)/", array(& $this, '_sub_jtosj'), $tmp);
			} elseif (ord(@$allchunks[$i][3])) { //Ⱦ�ѥ��ʤ˥ޥå�
	...
	function _utf8tosjis(&$str)
	{
		...
		$st = '';
		$max = preg_match_all('/'.$this->mbemu_internals['regex'][4].'/', $str, $allchars);  // ʸ���������ʬ��
		for ($i = 0; $i < $max; ++$i) {
			$num = $this->_utf8ucs2($allchars[0][$i]); //ucs2���ͤ���Ф�
			if ($num < 0x80)
				$st .= chr($num);
	...
	function _utf8toeuc(&$str)
	{
		...
		for ($i = 0; $i < $max; ++$i) {
			...
			if ($num < 0x80)
				$st .= chr($num);
			elseif ((0xff61 <= $num) && ($num <= 0xff9f)) //Ⱦ�ѥ���
	...
	function katakana_zenhan_EUC(&$str) {
		...
		$str = '';
		for ($i = 0; $i < $max; ++$i) {
			if ($num = ord($chars[1][$i])) //���ʤ˥ޥå��󥰤������
				$str .= chr(0x8e) . $this->mbemu_internals['kana_zenhan_convert'][$num];
			elseif ($num = ord($chars[2][$i])) //Ⱦ���Ѵ���ǽ���ü�ʸ���˥ޥå��������
	...
	function katakana_hanzen1_EUC(&$str) {	//����������򤹤���
	...
	function hiragana_hanzen1_EUC(&$str) {	//����������򤹤���
	...
	function katakana_hanzen2_EUC(&$str) {	//����������򤷤ʤ���
		$match = "[\xa1-\xfe][\xa1-\xfe]|[\x01-\x7f]|\x8e([\xa1-\xdf])";
		$max = preg_match_all("/$match/", $str, $chars);
		$str = '';
		for ($i = 0; $i < $max; ++$i) {
			if ($chars[1][$i]) //Ⱦ�ѥ��ʤ˥ޥå�
	...
	function hiragana_hanzen2_EUC(&$str) {	//����������򤷤ʤ���
		$match = "[\xa1-\xfe][\xa1-\xfe]|[\x01-\x7f]|\x8e([\xa1-\xdf])";
		$max = preg_match_all("/$match/", $str, $chars);
		$str = '';
		for ($i = 0; $i < $max; ++$i) {
	...
	function mb_decode_mimeheader($str)
	{
		...
		foreach ($lines as $line) {
			if ($line != "") {
				$line = preg_replace("/<[\w\-+\.]+\@[\w\-+\.]+>/","", $line); //�᡼�롦���ɥ쥹����ä�
	...
	function _sub_quoted_printable_encode($str, $encoding, $maxline, $linefeed)
	{
		...
		switch ($e = $this->mbemu_internals['encoding'][$encoding]) {
			...
			case 3 : //jis
				$max = preg_match_all('/'.$this->mbemu_internals['regex'][3].'/', $str, $allchunks, PREG_SET_ORDER);  // ʸ���老�Ȥ������ʬ��
				$st = ''; // quoted printable�Ѵ����ʸ����
				$len = $maxline;  // ���ιԤ��ɲò�ǽ�ʥХ��ȿ�
				$needterminate = FALSE; //�Ǹ�˥��������ץ������󥹤�ɬ�פ��ɤ���
				for ($i = 0; $i < $max; ++$i) {
					if (ord($allchunks[$i][1])) { //�ѿ��˥ޥå�
						if ($needterminate) {
							$st .= '=1B=28B';
	...
	function _sub_encode_base64($str, $encoding, $maxline , $linefeed)
	{
		switch ($e = $this->mbemu_internals['encoding'][$encoding]) {
			...
			case 8 : //utf16BE
				...
				break;
			case 3 : //jis
				$max = preg_match_all('/'.$this->mbemu_internals['regex'][3].'/', $str, $allchunks);  // ʸ���老�Ȥ������ʬ��
				$st = ''; // BASE64�Ѵ����ʸ����
				$maxbytes = floor($maxline * 3 / 4);  //1�Ԥ��Ѵ���ǽ�ʥХ��ȿ�
				$len = $maxbytes;  // ���ιԤ��ɲò�ǽ�ʥХ��ȿ�
				$line = '';  //1��ʬ���Ѵ�����ʸ����
				$needterminate = FALSE; //�Ǹ�˥��������ץ������󥹤�ɬ�פ��ɤ���
				for ($i = 0; $i < $max; ++$i) {
					if (ord($allchunks[1][$i])) { //�ѿ��˥ޥå�
						if ($needterminate) {
							$line .= chr(0x1B).'(B';
							$len -= 3;
						}
						$tmpstr = $allchunks[1][$i];  //�ɲä���ʸ����
						$l = strlen($tmpstr);  //�ɲä���ʸ�����Ĺ��
						while ($l > $len) {
						...
						$len -= $l;
						$needterminate = FALSE;
					} elseif (ord($allchunks[2][$i])) { //�����˥ޥå�
						$tmpstr = substr($allchunks[0][$i], 3);
						if ($len < 8) { //ʸ�����ɲä���Τ˺���8�Х���ɬ�פʤΤ�
							if ($needterminate)
								$line .= chr(0x1B).'(B';
							$st .= base64_encode($line).$linefeed;
							$len = $maxbytes;
					...
					} elseif (ord($allchunks[3][$i])) { //Ⱦ�ѥ��ʤ˥ޥå�
						$tmpstr = $allchunks[3][$i];
						if ($len < 7) { //ʸ�����ɲä���Τ˺���7�Х���ɬ�פʤΤ�
							if ($needterminate)
								$line .= chr(0x1B).'(B';
							$st .= base64_encode($line).$linefeed;
							$len = $maxbytes;
