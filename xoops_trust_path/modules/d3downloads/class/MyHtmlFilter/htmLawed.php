<?php
/* 
 * class MyHtmlFilter based htmLawed 1.1.7.2
 */
...
if( ! class_exists( 'MyHtmlFilter' ) ) {
	...
	class MyHtmlFilter
	{
		...
		function MyHtmlFilter($cf = 1, $spec = array())
		{
			$this->cf = is_array($cf) ? $cf : array();
			$this->spec = is_array($spec) ? $spec : $this->hl_spec($spec);
			$this->elements = array(
				'a'=>1,
				'abbr'=>1,
				'acronym'=>1,
				...
				'ul'=>1,
				'var'=>1
			); // 86 incl. deprecated, embed, ruby set
			$this->deny_attribute = array(
				'onblur'=>1,
				'onchange'=>1,
				'onclick'=>1,
				'ondblclick'=>1,
		...
		function htmLawed($t, $C=1, $S=array()){
			...
			if(isset($C['deny_attribute']['on*'])){
			 unset($C['deny_attribute']['on*']);
				$C['deny_attribute'] += $this->deny_attribute;
			}
			// config URL
			...
			foreach($aA as $k=>$v){
			 if((!isset($C['deny_attribute'][$k]) or isset($rl[$k])) && ((!isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e])))){
			  ...
			  if($k == 'style' && !$C['style_pass']){
			   ...
			   // ===== add by photosite ====================
			   if(!preg_match('`^('.implode('|',$this->style_list).'):`i', $v)){$v='';}
			   // ===== end photosite =======================
